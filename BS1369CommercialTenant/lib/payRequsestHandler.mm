...
{
    //订单标题，展示给用户
    NSString *order_name    = @"白送网星币充值";
    //订单金额,单位（分）
//    _order_price   = @"1";//1分钱测试
    NSString *attach    = [[NSUserDefaults standardUserDefaults] valueForKey:@"id"];
    //================================
    //预付单参数订单设置
    //================================
    srand( (unsigned)time(0) );
    NSString *noncestr  = [NSString stringWithFormat:@"%d", rand()];
#pragma mark - 流水号随机数生成
//    生成年月日时分秒加5位随机数
...
//    NSLog(@"最后年月日时分秒拼接的结果--->%@",lastTime);
#pragma mark - 
    NSString *orderno   = lastTime;
//    NSLog(@"流水号的结果--->%@",orderno);
    NSMutableDictionary *packageParams = [NSMutableDictionary dictionary];
    
    [packageParams setObject: appid             forKey:@"appid"];//开放平台appid
    [packageParams setObject: attach             forKey:@"attach"];//新加的参数   不知道是啥
    ...
    [packageParams setObject: _order_price       forKey:@"total_fee"];       //订单金额，单位为分
